...
public class FileSweeper {
    // 매일 정해진 시간에 스케줄러 실행
    // 매일 해당 날짜의 경로에 파일(DB)과 폴더 안의 파일이 일치하는지 비교
    // DB == 폴더의 파일이 일치하는지 확인
    // 일치하는 파일만 남기고(DB == file),
    // 일치하지 않는 파일(DB != file)은 삭제 (폴더에서 삭제)

    private final BoardService boardService;
    private  final String BASE_PATH = "D:\\web_0826_khb\\_myProject\\_java\\_fileUpload\\";
    ...
    public void fileSweeper(){
        ...
        // D:\web_0826_khb\_myProject\_java\_fileUpload\2025\12\26
        File dir = Paths.get(BASE_PATH+today).toFile();

        // 경로 안에 있는 파일을 가져오기 (배열로 리턴)
        File[] allFileObject = dir.listFiles();

        // allfileObject와 DB에 있는 파일을 비교
